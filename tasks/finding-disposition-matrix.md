...
| The token scan is **disabled** exactly when a DSN is configured | **REAL — OPEN**, fix shape named | A DSN build gets no `@sentry` verification at all while being stamped `true` — the mirror of the false positive already closed. **The scan is gated `!sentryConfigured` because absence cannot be asserted in a build that deliberately includes the SDK. The missing piece is the INVERSE assertion: when a DSN IS configured, assert `sentry_key\|sentry_client` is PRESENT, proving the SDK actually got bundled.** Unexercised today — no shipping build configures a DSN — and newly load-bearing for whichever release first does |
